...
/**
 * CHECKOUT. Order bump is offered BEFORE the order is placed.
 * Phase 1 is manual wire, so nothing is charged here: we record the order and
 * carry the visitor through the add-on steps, then show banking details.
 */
export default function CheckoutPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // Best effort: never block the funnel if Supabase is not configured yet.
    await createOrder({
      ...
      <div className="mx-auto grid max-w-[900px] gap-6 md:grid-cols-[1.1fr_0.9fr]">
        ...
        <form onSubmit={onSubmit} className="rounded-2xl border border-midnight/10 bg-white p-6 md:p-8">
          ...
          <Button type="submit" disabled={busy} className="mt-6 w-full">
            {busy ? "One moment…" : "Place my order"} <span aria-hidden="true">&#8599;</span>
          </Button>
        ...
        <aside className="h-max rounded-2xl bg-midnight p-6 text-cream md:p-8">
          ...
          <p className="mt-2 text-xs leading-relaxed text-cream/60">
            Nothing to pay now. Once you place your order, we&rsquo;ll message you on WhatsApp with
            your workshop details and how to settle.
          </p>
